...
const IndividualChat = () => {
    ...
    const loadConversationFromUrl = async () => {
      ...
      try {
        ...
        if (convData.agent_type === 'predefined') {
          ...
          const { data: customAgentData, error: customAgentError } = await supabase.from('custom_agents').select('id, name, title, description, persona, emoji, avatar_url').eq('id', convData.agent_id).single();
          if (customAgentError || !customAgentData) throw new Error('Clone customizado não encontrado.');
          agentData = { ...customAgentData, avatar: customAgentData.avatar_url || '/placeholder.svg', type: 'custom' };
        }
  ...
  useEffect(() => {
    const fetchCustomAgents = async () => {
      ...
      try {
        const { data, error } = await supabase.from('custom_agents').select('id, name, title, description, persona, created_at, emoji, avatar_url').eq('user_id', session.user.id).order('created_at', { ascending: false });
  ...
  useEffect(() => {
    ...
    const allAgents = [
      ...
      ...customAgents.map(clone => ({
        id: clone.id, name: clone.name, avatar: clone.avatar_url || '/placeholder.svg', title: clone.title || 'Clone Customizado', description: clone.description || 'Um especialista de IA criado por você.', tags: ['Customizado'], fidelity: 'Alta' as const, customizable: true, type: 'custom' as const, emoji: clone.emoji
      }))
    ];
